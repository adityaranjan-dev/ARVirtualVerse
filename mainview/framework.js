/*
ARVirtualVerse Main GUI (display when boot finish)
*/
        ...
        () =>
            html2canvas(mydiv, { backgroundColor: null }).then(function (
                ...
                var boomOption2 = {
                    ...
                    // particle size
                    radius: 3,
                    // Minimum horizontal injection speed
                    minVx: -20,
                    // Maximum horizontal jet speed
                    maxVx: 25,
                    // Minimum vertical jet speed
                    minVy: -25,
                    // Maximum vertical jet speed
                    maxVy: 0.1,
                    speed: 10,
                    onBoomEnd: function () {
                        targetBak.remove();
...
if (typeof require != "undefined") {
    ...
    var f = async () => {
        ...
        console.log(theme)
        ipcRenderer.send('tabChanged',window.arvirtualverseApp.tab,document.body.style.getPropertyValue('--md-sys-color-primary'),document.body.style.getPropertyValue('--md-sys-color-primary-container'));
    };
    f();
    ...
    //Block related event default behavior
...
ipcRenderer.on("switch-tab", (ev, data) => {
    window.arvirtualverseApp.tab = data;
});
...
window.startMocap = async function (e) {
    if (process.platform == "darwin" && app.videoSource == "camera")
        if (
            ...
            if (!(await remote.systemPreferences.askForMediaAccess("camera"))) {
                alert("Requires permission to use camera");
                return;
            }
        }
    if (e.innerHTML.indexOf(app.language.tabMocap.start) != -1) {
        ...
        if (window.arvirtualverseApp.settings.performance.useDescrertionProcess) {
            ...
            if (window.arvirtualverseApp.settings.dev.openDevToolsWhenMocap)
        ...
        if (window.arvirtualverseApp.settings.forward.enableForwarding)
...
window.checkUpdate = () => {
    if (window.arvirtualverseApp.checkingUpdate || window.arvirtualverseApp.isLatest) return;
    window.arvirtualverseApp.checkingUpdate = true;

    // version check options (for details see below)
const options = {
    repo: 'ARVirtualVerse',                    // repository name
    owner: 'B29',                               // repository owner
    currentVersion: 'v' + window.arvirtualverseApp.appVersion,                       // your app's current version
  };
  
  versionCheck(options, function (error, update) { // callback function
    window.arvirtualverseApp.updateError = null
    if (error) {
        window.arvirtualverseApp.updateError = error
        window.arvirtualverseApp.checkingUpdate = false
        return
    }
    if (update) { // print some update info if an update is available
      console.log('An update is available! ' + update.name);
      window.arvirtualverseApp.hasUpdate = update
    } else {
        window.arvirtualverseApp.isLatest = true
    }
...
window.openInGithub = () =>
    remote.shell.openExternal("https://github.com/adityaranjan-dev/ARVirtualVerse/releases");

if(!window.arvirtualverseApp.disableAutoUpdate) window.checkUpdate()

window.addEventListener('scroll',function(e){
    if(window.pageYOffset > 10){
        window.arvirtualverseApp.showLine = true
    }else{
        window.arvirtualverseApp.showLine = false
    }
  })